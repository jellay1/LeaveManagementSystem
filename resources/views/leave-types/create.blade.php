...
<div class="max-w-2xl">
    <div class="mb-8">
        ...
        <h1 class="text-4xl font-bold text-slate-900">Add Leave Type</h1>
        <p class="text-slate-600 mt-2">Create a leave category so employees can request time off with the right approval rules.</p>
    </div>
        ...
        <form method="POST" action="{{ route('leave-types.store') }}" class="space-y-6">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="text" id="name" name="name" value="{{ old('name') }}" class="w-full px-4 py-2.5 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-slate-900 focus:border-transparent transition @error('name') border-red-500 @enderror" placeholder="e.g. Vacation, Sick Leave" required>
                ...
                <div>
                    ...
                    <input type="number" id="annual_allocation" name="annual_allocation" value="{{ old('annual_allocation') }}" class="w-full px-4 py-2.5 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-slate-900 focus:border-transparent transition @error('annual_allocation') border-red-500 @enderror" placeholder="e.g. 20" required>
            ...
            <div class="p-4 bg-slate-50 rounded-lg border border-slate-200 flex gap-3">
                ...
                <input type="checkbox" id="requires_approval" name="requires_approval" value="1" {{ old('requires_approval') ? 'checked' : '' }} class="mt-1 w-5 h-5 text-slate-900 border-slate-300 rounded focus:ring-2 focus:ring-slate-900">
